...
export function renderBanner({ version, cohort }) {
  ...
  lines.push(`    ${c.amber('whoami')}              ${c.dim(t('banner.menu.whoami'))}`);
  lines.push(`    ${c.amber('sponsor')}             ${c.dim(t('banner.menu.sponsor'))}`);
  lines.push(`    ${c.amber('reset [--yes]')}       ${c.dim(t('banner.menu.reset'))}`);
...
export function renderHelp({ version }) {
  ...
  L.push(`  whoami                       ${t('banner.menu.whoami')}`);
  L.push(`  sponsor                      ${t('banner.menu.sponsor')}`);
  L.push(`  reset [--yes]                ${t('banner.menu.reset')}`);
  L.push(`  doctor --json                doctor as structured JSON`);
  ...
  L.push('  --json              on doctor: emit structured JSON to stdout');
  L.push('  --yes               on reset: skip the confirmation prompt');
